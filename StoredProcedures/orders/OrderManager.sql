...
BEGIN
	...
	SET @_lastorder := (SELECT `order` FROM kraken.orders ORDER BY timestamp DESC LIMIT 1);
	...
	-- buy top
	IF  (@_sma5mintrending = "buy") AND (@_sma20mintrending = "buy") AND (@_sma60mintrending = "buy") AND (@_sma24htrending = "buy") AND (@_volumeflowtrending = "volatile") AND (@_lastorder <> "buy") THEN 
		SET _order = "buy";
		SET _type= "top";
	END IF;
	
	-- sell standard
	IF  (@_sma5mintrending = "sell") AND (@_sma20mintrending = "sell") AND (@_sma60mintrending = "sell") AND (@_sma24htrending = "sell") AND (@_lastorder <> "sell") THEN 
		SET _order = "sell";
		SET _type= "std";
    END IF;
